...
async function checkAndPay() {
  ...
  if (error) console.error("Error inserting payment:", error);
      ...
      console.log("Successfully inserted another 300 payment for Emily.");
      
      const { data: existingPayments } = await supabase.from('fee_payments').select('amount_paid').eq('student_id', s.id);
      const totalPaid = existingPayments.reduce((acc, p) => acc + (p.amount_paid || 0), 0);
