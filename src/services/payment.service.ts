...
export const paymentService = {
   ...
   */
  async verifyPaymentOnServer(reference: string, studentId: string, termId: string, schoolId: string, amount?: number) {
    const { supabase } = await import('../lib/supabase')
    const { data, error } = await supabase.functions.invoke('verify-payment', {
      body: { reference, student_id: studentId, term_id: termId, school_id: schoolId, amount }
    })
