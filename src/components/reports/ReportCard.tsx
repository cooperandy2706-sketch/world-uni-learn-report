...
export default function ReportCard({
  ...
  const theme = settings?.report_theme ?? 'modern'
  const primary = settings?.primary_color ?? '#1e3a8a'
  const watermark = settings?.report_watermark_url
  ...
  return (
    <>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;500;600;700&family=Playfair+Display:wght@600;700&family=Inter:wght@400;500;600;700&display=swap');
        
        .rc-wrap { 
          font-family: ${isClassic ? '"Playfair Display", serif' : isProf ? '"Inter", sans-serif' : '"DM Sans", sans-serif'}; 
          font-size: 13.5px; color: #000; background: #fff; max-width: 800px; margin: 0 auto; padding: 12px 16px; box-sizing: border-box; line-height: 1.4; position: relative; 
          border: ${isClassic ? '3px double #000' : 'none'};
        }

        .rc-watermark {
          position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%) rotate(-30deg);
          width: 70%; height: 70%; pointer-events: none; opacity: 0.08; z-index: 0;
          display: flex; align-items: center; justify-content: center;
          background-image: url('${watermark}'); background-repeat: no-repeat; background-position: center; background-size: contain;
        }

        .rc-table { width: 100%; border-collapse: collapse; border: ${isBW || isClassic ? '1px solid #000' : 'none'}; z-index: 1; position: relative; }
        
        .rc-table th { 
          background: ${isBW ? '#f1f5f9' : isClassic ? '#fff' : isProf ? '#334155' : primary}; 
          color: ${isBW || isClassic ? '#000' : '#fff'}; 
          padding: 6px 8px; text-align: left; font-size: 10.5px; font-weight: 700; text-transform: uppercase; letter-spacing: .05em; white-space: nowrap; 
          border: ${isBW || isClassic ? '1px solid #000' : 'none'}; 
        }

        .rc-table td { 
          padding: 5px 8px; 
          border-bottom: ${isBW || isClassic ? '1px solid #000' : '.5px solid #e2e8f0'}; 
          font-size: 12px; color: #000; 
          border-right: ${isBW || isClassic ? '1px solid #000' : 'none'}; 
        }

        .rc-table tr:nth-child(even) td { background: ${isBW || isClassic || isProf ? 'none' : '#f8fafc'}; }
        
        .rc-header { 
          display: flex; align-items: center; gap: 20px; margin-bottom: 15px; 
          flex-direction: ${isClassic ? 'column' : 'row'};
      ...
      <div className="rc-wrap">
        <div style={{ filter: isFinancialHold ? 'blur(10px)' : 'none', pointerEvents: isFinancialHold ? 'none' : 'auto', transition: 'filter 0.3s ease' }}>
          {watermark && <div className="rc-watermark" />}
          ...
          {!hideSettings && (
            <div className="no-print" data-html2canvas-ignore="true" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12, paddingBottom: 8, borderBottom: '1px solid #f1f5f9' }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: primary }}>🖨️ Print Settings</div>
          ...
          <div className="rc-header">
            ...
            <div style={{ flex: 1 }}>
              ...
              {school?.motto && <div style={{ fontSize: 12, fontStyle: 'italic', color: '#64748b', marginTop: 2 }}>"{school.motto}"</div>}
          ...
          <div style={{ marginTop: 10, background: isBW ? '#000' : isClassic ? '#fff' : isProf ? '#1e293b' : primary, color: isClassic && !isBW ? '#000' : '#fff', border: isClassic ? '1px solid #000' : 'none', padding: '5px 0', borderRadius: 3, fontSize: 12, fontWeight: 700, textAlign: 'center', letterSpacing: '.08em', textTransform: 'uppercase' }}>
            {isClassic && <div style={{ borderBottom: '1px solid #000', margin: '0 20px 5px', opacity: 0.3 }} />}
            Student Report Card — {(term as any)?.name} &nbsp;·&nbsp; {(year as any)?.name}
            {isClassic && <div style={{ borderTop: '1px solid #000', margin: '5px 20px 0', opacity: 0.3 }} />}
          </div>
            ...
            <table className="rc-table">
              ...
              <tbody>
                {scores.length === 0 ? (
                  ...
                  const g = getGradeInfo(s.total_score ?? 0, scale)
                  return (
                    <tr key={s.id}>
          ...
          <div className="rc-section-gap" style={{ marginBottom: 8, padding: '5px 10px', background: '#f8fafc', borderRadius: 5, border: '.5px solid #e2e8f0' }}>
            ...
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
              {(scale && scale.length > 0 ? scale : GRADE_SCALE).map((g: any) => (
                <span key={g.grade || g.label} style={{ fontSize: 11.5, color: '#000' }}>
                  <strong style={{ color: isBW ? '#000' : (g.color || g.color_code) }}>{g.grade || g.label}</strong>: {g.min ?? g.min_score}–{g.max ?? g.max_score} ({g.label || g.remark})
                </span>
              ))}
            </div>
          </div>
          ...
          <div style={{ borderTop: isBW ? '3px solid #000' : '2px solid #1e3a8a', paddingTop: 7 }}>
            ...
            <p style={{ fontSize: 9, color: isBW ? '#000' : '#94a3b8', marginTop: 5, textAlign: 'center', borderTop: isBW ? '1px solid #000' : '.5px solid #f1f5f9', paddingTop: 4 }}>
              Generated by Acadera Report &nbsp;·&nbsp; {school?.name} &nbsp;·&nbsp; {(term as any)?.name} {(year as any)?.name}
            </p>
          </div>
        </div>
        ...
        {isFinancialHold && (
          <div style={{
            ...
            <div style={{ 
              width: 80, height: 80, background: '#fee2e2', color: '#dc2626', 
              borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center',
              fontSize: 40, marginBottom: 20, boxShadow: '0 8px 30px rgba(220,38,38,0.2)'
            }}>
              🔒
            </div>
            ...
            <div style={{ marginTop: 24, display: 'flex', gap: 12 }}>
              <div style={{ background: '#1e1b4b', color: '#fff', padding: '12px 24px', borderRadius: 12, fontWeight: 700, fontSize: 14 }}>
                Contact Bursar
              </div>
            </div>
          </div>
