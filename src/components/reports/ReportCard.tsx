  ...
  return (
    <>
      <style>{`
        ...
        .rc-wrap { font-family:'DM Sans',sans-serif; font-size:12px; color:#0f172a; background:#fff; max-width:740px; margin:0 auto; padding:12px 16px; box-sizing:border-box; }
        .rc-table { width:100%; border-collapse:collapse; }
        .rc-table th { background:#1e3a8a; color:#fff; padding:5px 7px; text-align:left; font-size:9.5px; font-weight:600; text-transform:uppercase; letter-spacing:.05em; white-space:nowrap; }
        .rc-table td { padding:4px 7px; border-bottom:.5px solid #e2e8f0; font-size:11px; }
        .rc-table tr:nth-child(even) td { background:#f8fafc; }
        .rc-select { border:1px solid #e2e8f0; border-radius:5px; padding:3px 6px; font-size:11px; font-family:'DM Sans',sans-serif; width:100%; background:#fff; color:#0f172a; }
        ...
        @media print {
          ...
          .rc-wrap { width:194mm !important; max-width:194mm !important; padding:0 !important; margin:0 !important; font-size:10.5px !important; }
          .rc-section-gap { margin-bottom:5px !important; }
          .rc-table td,.rc-table th { padding:3px 5px !important; }
          .rc-table td { font-size:9.5px !important; }
          .rc-table th { font-size:8px !important; }
          .no-print { display:none !important; }
          .rc-att-input { border:none !important; background:transparent !important; font-size:10px !important; }
          .rc-summary-tile { padding:5px 8px !important; }
          .rc-big { font-size:13px !important; }
        }
      `}</style>
      ...
      <div className="rc-wrap">
        ...
        <div className="rc-section-gap" style={{ textAlign:'center', borderBottom:'2.5px solid #1e3a8a', paddingBottom:10, marginBottom:10 }}>
          <div style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:12 }}>
            {school?.logo_url
              ...
              : <div style={{ width:48, height:48, borderRadius:'50%', background:'#eff6ff', display:'flex', alignItems:'center', justifyContent:'center', fontSize:20, flexShrink:0 }}>🏫</div>
            }
            <div>
              <div style={{ fontFamily:'"Playfair Display",serif', fontSize:18, fontWeight:700, color:'#1e3a8a', lineHeight:1.2 }}>
              ...
              {school?.phone   && <div style={{ fontSize:10, color:'#64748b' }}>Tel: {school.phone}</div>}
            </div>
          </div>
          <div style={{ marginTop:8, background:'#1e3a8a', color:'#fff', padding:'4px 0', borderRadius:3, fontSize:11, fontWeight:700, letterSpacing:'.08em', textTransform:'uppercase' }}>
            Student Report Card — {(term as any)?.name} &nbsp;·&nbsp; {(year as any)?.name}
          </div>
        </div>

        {/* ── STUDENT INFO ── */}
        <div className="rc-section-gap" style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:'3px 16px', marginBottom:8, padding:'7px 10px', background:'#f8fafc', borderRadius:6, border:'.5px solid #e2e8f0' }}>
          {[
            ...
            <div key={label} style={{ display:'flex', gap:4, alignItems:'baseline' }}>
              <span style={{ fontSize:10, color:'#64748b', fontWeight:600, flexShrink:0, minWidth:74 }}>{label}:</span>
              <span style={{ fontSize:11.5, fontWeight:600, color:'#0f172a' }}>{value}</span>
            </div>
          ))}
        </div>

        {/* ── ATTENDANCE ── */}
        <div className="rc-section-gap" style={{ marginBottom:8, padding:'7px 10px', background:'#eff6ff', borderRadius:6, border:'.5px solid #dbeafe' }}>
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:8 }}>
            <span style={{ fontSize:10, fontWeight:700, color:'#1e40af', textTransform:'uppercase', letterSpacing:'.05em' }}>Attendance</span>
            {attendance ? (
        ...
        <div className="rc-section-gap" style={{ marginBottom:8 }}>
          <table className="rc-table">
            <thead>
              <tr>
                <th>Subject</th>
                <th style={{ textAlign:'center', width:65 }}>Class Score<br/><span style={{ fontSize:8, opacity:.8 }}>(50%)</span></th>
                <th style={{ textAlign:'center', width:65 }}>Exam Score<br/><span style={{ fontSize:8, opacity:.8 }}>(50%)</span></th>
                ...
                return (
                  <tr key={s.id}>
                    ...
                    <td style={{ textAlign:'center' }}>
                      <span style={{ fontWeight:700, color:g.color, background:g.color+'18', padding:'1px 5px', borderRadius:4, fontSize:11 }}>{g.grade}</span>
                    </td>
        ...
        <div className="rc-section-gap" style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:8, marginBottom:8 }}>
          {[
            ...
            <div key={label} className="rc-summary-tile" style={{ background:'#f8fafc', border:'.5px solid #e2e8f0', borderRadius:6, padding:'7px 10px', textAlign:'center' }}>
              <div style={{ fontSize:9, color:'#64748b', fontWeight:600, textTransform:'uppercase', letterSpacing:'.05em', marginBottom:2 }}>{label}</div>
        ...
        <div className="rc-section-gap" style={{ marginBottom:8, padding:'5px 10px', background:'#f8fafc', borderRadius:5, border:'.5px solid #e2e8f0' }}>
          <div style={{ fontSize:9, fontWeight:700, color:'#64748b', textTransform:'uppercase', letterSpacing:'.06em', marginBottom:4 }}>Grading Scale</div>
        ...
        <div className="rc-section-gap" style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, marginBottom:8 }}>
          <div style={{ border:'.5px solid #e2e8f0', borderRadius:6, padding:'8px 10px' }}>
            <div style={{ fontSize:9.5, fontWeight:700, color:'#64748b', textTransform:'uppercase', letterSpacing:'.06em', marginBottom:5 }}>Class Teacher's Remarks</div>
            {readonly
          ...
          <div style={{ border:'.5px solid #e2e8f0', borderRadius:6, padding:'8px 10px' }}>
            <div style={{ fontSize:9.5, fontWeight:700, color:'#64748b', textTransform:'uppercase', letterSpacing:'.06em', marginBottom:5 }}>Headteacher's Remarks</div>
            {readonly
        ...
        <div style={{ borderTop:'2px solid #1e3a8a', paddingTop:7 }}>
          ...
          <p style={{ fontSize:9, color:'#94a3b8', marginTop:5, textAlign:'center', borderTop:'.5px solid #f1f5f9', paddingTop:4 }}>
