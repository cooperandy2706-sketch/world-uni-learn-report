// src/pages/admin/DriverStaffPage.tsx
// Admin page to create and manage driver staff accounts
...
import { Plus, Trash2, KeyRound, Shield, Eye, EyeOff, Users, Bus, Activity } from 'lucide-react'
...
export default function DriverStaffPage() {
  ...
  const [activeMainTab, setActiveMainTab] = useState<'accounts' | 'fleet' | 'live'>('accounts')
  const [saving, setSaving] = useState(false)
  const [newPw, setNewPw] = useState('')
  const [showPw, setShowPw] = useState(false)
  const [form, setForm] = useState({ full_name: '', email: '', phone: '', password: '' })

  const { data: driverStaff = [], isLoading } = useQuery({
    queryKey: ['driver-staff', schoolId],
    queryFn: async () => {
      const { data } = await supabase.from('users').select('*').eq('school_id', schoolId).eq('role', 'driver').order('full_name')
      return data ?? []
    },
    enabled: !!schoolId,
  })
  ...
  async function handleCreate() {
    ...
    setSaving(true)
    try {
      const pw = form.password || 'driver123'
      ...
      qc.invalidateQueries({ queryKey: ['driver-staff'] })
      setCreateModal(false)
      setForm({ full_name: '', email: '', phone: '', password: '' })
    } catch (e: unknown) {
      toast.error(e instanceof Error ? (e.message ?? 'Failed to create driver account') : 'Failed to create driver account')
    } finally {
      setSaving(false)
    }
  }
  ...
  async function handleDelete(s: any) {
    if (!confirm(`Remove driver account for ${s.full_name}? This will deactivate their login.`)) return
    try {
      const { error, data } = await supabase.functions.invoke('admin-ops', {
        body: { action: 'delete-user', payload: { target_user_id: s.id, role: 'driver' } }
      })
      if (error) throw error
      if (data?.error) throw new Error(String(data.error))
    ...
    } catch (e: unknown) { toast.error(e instanceof Error ? e.message : 'Failed to remove driver account') }
  }
    ...
    <>
      ...
      <div style={{ fontFamily: '"DM Sans",system-ui,sans-serif', animation: '_sec_fi .4s ease' }}>
        <div style={{ marginBottom: 24, display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
          <div>
            <h1 style={{ fontFamily: '"Playfair Display",serif', fontSize: 26, fontWeight: 700, color: '#0f172a', margin: 0 }}>Transport & Drivers</h1>
            <p style={{ fontSize: 13, color: '#64748b', marginTop: 3 }}>Manage school buses, routes, and driver accounts</p>
          </div>
          {activeMainTab === 'accounts' && <Btn onClick={() => setCreateModal(true)}><Plus size={14} /> Add driver account</Btn>}
        </div>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 24, borderBottom: '1px solid #e2e8f0' }}>
          <button 
            ...
            style={{ padding: '12px 24px', border: 'none', background: 'transparent', fontWeight: 700, fontSize: 15, color: activeMainTab === 'accounts' ? '#1d4ed8' : '#64748b', borderBottom: activeMainTab === 'accounts' ? '3px solid #1d4ed8' : '3px solid transparent', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8 }}
          >
            <Users size={18} /> Driver Accounts
          </button>
          <button 
            onClick={() => setActiveMainTab('fleet')}
            style={{ padding: '12px 24px', border: 'none', background: 'transparent', fontWeight: 700, fontSize: 15, color: activeMainTab === 'fleet' ? '#1d4ed8' : '#64748b', borderBottom: activeMainTab === 'fleet' ? '3px solid #1d4ed8' : '3px solid transparent', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8 }}
          >
            <Bus size={18} /> Fleet Management
          </button>
          <button 
            onClick={() => setActiveMainTab('live')}
            style={{ padding: '12px 24px', border: 'none', background: 'transparent', fontWeight: 700, fontSize: 15, color: activeMainTab === 'live' ? '#1d4ed8' : '#64748b', borderBottom: activeMainTab === 'live' ? '3px solid #1d4ed8' : '3px solid transparent', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8 }}
          >
        ...
        <div style={{ background: 'linear-gradient(135deg,#f8fafc,#f1f5f9)', border: '1px solid #e2e8f0', borderRadius: 14, padding: '14px 18px', marginBottom: 22, display: 'flex', gap: 12, alignItems: 'flex-start' }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 12, color: '#475569', margin: 0 }}>driver staff can manage visitor logs, track student gate entry/exit, and view emergency contacts. They cannot access academic or financial records.</p>
          </div>
        </div>
      ...
      {/* Create Modal */}
      <Modal open={createModal} onClose={() => setCreateModal(false)} title="Create Security Account" subtitle="Setup login for gate/drivers" size="sm"
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          {[
            ...
            { label: 'Email Address *', key: 'email', placeholder: 'driver@school.edu.gh', type: 'email' },
            { label: 'Phone Number', key: 'phone', placeholder: '024 000 0000', type: 'tel' },
            { label: 'Password', key: 'password', placeholder: 'Leave blank for "driver123"', type: 'password' },
          ].map(f => (
            <div key={f.key}>
          ...
          <div style={{ background: '#fef3c7', border: '1px solid #fde68a', borderRadius: 9, padding: '10px 14px', fontSize: 12, color: '#92400e' }}>
            <strong>Default password:</strong> driver123
          </div>
        </div>
      </Modal>
