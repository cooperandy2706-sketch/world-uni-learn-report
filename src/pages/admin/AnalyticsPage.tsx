    ...
    <>
      ...
      <div style={{ fontFamily: '"DM Sans",system-ui,sans-serif', animation: '_afadeIn 0.4s ease' }}>
        ...
        <div style={{ marginBottom: 24, display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
          <div>
            <h1 style={{ fontFamily: '"Playfair Display",serif', fontSize: 26, fontWeight: 700, color: 'var(--text-main)', margin: 0 }}>Analytics</h1>
        ...
        <div style={{ display: 'flex', gap: 12, marginBottom: 20, background: 'var(--bg-card)', padding: 4, borderRadius: 12, border: '1.5px solid #f0eefe', width: 'fit-content' }}>
          {[
        ...
        {/* Class selector */}
        <div style={{ background: 'var(--bg-card)', borderRadius: 14, padding: '16px 20px', border: '1.5px solid #f0eefe', marginBottom: 22, display: 'flex', flexWrap: 'wrap', gap: 14, alignItems: 'flex-end' }}>
          <div style={{ flex: '1 1 220px' }}>
            <label style={{ display: 'block', fontSize: 11, fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: 5 }}>Select Class</label>
            <select value={selectedClass} onChange={e => setSelectedClass(e.target.value)}
              onFocus={() => setClassFocused(true)} onBlur={() => setClassFocused(false)}
              style={{ width: '100%', padding: '9px 12px', borderRadius: 9, fontSize: 13, border: `1.5px solid ${classFocused ? '#7c3aed' : '#e5e7eb'}`, boxShadow: classFocused ? '0 0 0 3px rgba(109,40,217,0.1)' : 'none', outline: 'none', background: '#faf5ff', color: 'var(--text-main)', fontFamily: '"DM Sans",sans-serif', cursor: 'pointer' }}>
        ...
        {loading && (
          <div style={{ display: 'flex', justifyContent: 'center', padding: '60px 0', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
            ...
            <p style={{ fontSize: 13, color: 'var(--text-subtle)' }}>Crunching the numbers…</p>
          </div>
        )}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20, animation: '_afadeIn 0.5s ease' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(150px,1fr))', gap: 14 }}>
              {[
                ...
                <div key={i} style={{ background: 'var(--bg-card)', borderRadius: 14, padding: '16px 18px', border: '1.5px solid #f0eefe', boxShadow: '0 1px 4px rgba(109,40,217,0.06)', animation: `_afadeUp 0.4s ease ${i * 0.07}s both` }}>
                    ...
                    return (
                      <div key={r.student_id || r.id} 
                        ...
                        <div style={{ flex: 1 }}>
                          ...
                          <div style={{ fontSize: 11, color: 'var(--text-subtle)' }}>Performance: {g.label}</div>
                        </div>
                 ...
                 <div style={{ background: 'var(--bg-card)', borderRadius: 8, padding: '20px', border: '1.5px solid #f0eefe', boxShadow: '0 1px 4px rgba(109,40,217,0.06)', minHeight: 300 }}>
                    {!selectedStudentId ? (
                      <div style={{ height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center' }}>
                         ...
                         <h4 style={{ fontSize: 14, fontWeight: 700, color: 'var(--text-main)' }}>Student Drill-down</h4>
                         <p style={{ fontSize: 12, color: 'var(--text-subtle)', maxWidth: 180 }}>Click a student on the left to see their performance in all subjects.</p>
                      </div>
                    ) : (
                      <div style={{ animation: '_afadeIn 0.3s ease' }}>
                        ...
                        {studentPerformance && studentPerformance.length > 0 ? (
                          ...
                          <div style={{ textAlign: 'center', padding: '20px 0' }}>
                            <p style={{ fontSize: 12, color: 'var(--text-subtle)' }}>No subject scores found for this student.</p>
                          </div>
                        )}
                      </div>
                    )}
                          ...
                          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                            <div style={{ flex: 1, minWidth: 0 }}>
                              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                                <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-main)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{s.name}</span>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10 }}>
                    {[
                      ...
                      <div key={seg.label} style={{ background: seg.bg, borderRadius: 12, padding: '12px', border: `1px solid ${seg.color}20`, textAlign: 'center' }}>
                        ...
                        <div style={{ fontSize: 10, color: 'var(--text-muted)', fontWeight: 600 }}>{seg.label}</div>
                      </div>
                    ))}
                  </div>
               </div>
