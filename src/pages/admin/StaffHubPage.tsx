import { useState } from 'react'
import StaffDirectoryPage from './StaffDirectoryPage'
import StaffRequestsPage from './StaffRequestsPage'
import AdminTasksPage from './AdminTasksPage'
import { Users, ClipboardList, MessageSquare } from 'lucide-react'

export default function StaffHubPage() {
  const [activeTab, setActiveTab] = useState<'directory' | 'requests' | 'tasks'>('directory')

  const tabs = [
    { key: 'directory', label: 'Staff Directory', icon: <Users size={16} /> },
    { key: 'requests',  label: 'Staff Requests',  icon: <MessageSquare size={16} /> },
    { key: 'tasks',     label: 'Task Assignments', icon: <ClipboardList size={16} /> },
  ] as const

  return (
    <div style={{ padding: '28px 32px', maxWidth: 1200, margin: '0 auto', fontFamily: '"DM Sans", sans-serif' }}>
      <div style={{ marginBottom: 24 }}>
        <h1 style={{ fontFamily: '"Playfair Display", serif', fontSize: 28, fontWeight: 700, color: 'var(--text-main)', margin: '0 0 4px' }}>
          Staff & HR Hub
        </h1>
        <p style={{ fontSize: 13, color: 'var(--text-muted)', margin: 0 }}>
          Manage staff profiles, leave requests, and administrative tasks.
        </p>
      </div>

      {/* Tabs */}
      <div style={{ display: 'flex', gap: 4, marginBottom: 24, borderBottom: '2px solid var(--border-color)', overflowX: 'auto', paddingBottom: 0 }}>
        {tabs.map(t => (
          <button
            key={t.key}
            onClick={() => setActiveTab(t.key)}
            style={{
              padding: '10px 18px', border: 'none', background: 'transparent', fontWeight: 700,
              fontSize: 13, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 7,
              whiteSpace: 'nowrap', fontFamily: '"DM Sans", sans-serif',
              color: activeTab === t.key ? '#6d28d9' : 'var(--text-muted)',
              borderBottom: activeTab === t.key ? '3px solid #6d28d9' : '3px solid transparent',
              marginBottom: -2,
              transition: 'color 0.15s',
            }}
          >
            {t.icon} {t.label}
          </button>
        ))}
      </div>

      <div style={{ animation: 'fadeIn 0.25s ease' }}>
        {activeTab === 'directory' && <StaffDirectoryPage />}
        {activeTab === 'requests'  && <StaffRequestsPage />}
        {activeTab === 'tasks'     && <AdminTasksPage />}
      </div>
    </div>
  )
}
