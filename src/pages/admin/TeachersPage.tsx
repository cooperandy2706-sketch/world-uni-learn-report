...
export default function TeachersPage() {
  ...
  async function addAssignment() {
    ...
    newClassIds.forEach(cId => {
      newSubjectIds.forEach(subId => {
        inserts.push({
          teacher_id: assigningTeacher.id, class_id: cId, subject_id: subId,
    ...
    if (error) {
      if (error.message.includes('unique constraint') || error.code === '23505') {
        toast.error('One or more of these subjects are already assigned to this teacher!');
      } else {
            ...
            <div style={{ background: '#f8fafc', borderRadius: 12, padding: '14px 16px', border: '.5px solid #e2e8f0' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 10 }}>
                <div style={{ gridColumn: 'span 2' }}>
                  <label style={{ fontSize: 11, fontWeight: 600, color: '#374151', display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
                    <span>Classes</span>
                    <button onClick={() => {
                      if (newClassIds.length === (classes as any[]).length) setNewClassIds([])
                      else setNewClassIds((classes as any[]).map(c => c.id))
                    }} style={{ fontSize: 10, color: '#6d28d9', background: 'none', border: 'none', padding: 0, cursor: 'pointer', fontWeight: 700 }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(130px, 1fr))', gap: 6, maxHeight: 110, overflowY: 'auto', background: '#fff', border: '1.5px solid #e5e7eb', borderRadius: 8, padding: 8 }}>
                    {(classes as any[]).map(c => (
                      <label key={c.id} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, cursor: 'pointer' }}>
                        <input type="checkbox" checked={newClassIds.includes(c.id)} onChange={e => {
                          if (e.target.checked) setNewClassIds(p => [...p, c.id])
                          else setNewClassIds(p => p.filter(id => id !== c.id))
                        }} />
                ...
                <div style={{ gridColumn: 'span 2', marginTop: 6 }}>
                  <label style={{ fontSize: 11, fontWeight: 600, color: '#374151', display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
                    <span>Subjects</span>
                    <button onClick={() => {
                      if (newSubjectIds.length === (subjects as any[]).length) setNewSubjectIds([])
                      else setNewSubjectIds((subjects as any[]).map(s => s.id))
                    }} style={{ fontSize: 10, color: '#6d28d9', background: 'none', border: 'none', padding: 0, cursor: 'pointer', fontWeight: 700 }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(130px, 1fr))', gap: 6, maxHeight: 110, overflowY: 'auto', background: '#fff', border: '1.5px solid #e5e7eb', borderRadius: 8, padding: 8 }}>
                    {(subjects as any[]).map(s => (
                      <label key={s.id} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, cursor: 'pointer' }}>
                        <input type="checkbox" checked={newSubjectIds.includes(s.id)} onChange={e => {
                          if (e.target.checked) setNewSubjectIds(p => [...p, s.id])
                          else setNewSubjectIds(p => p.filter(id => id !== s.id))
                        }} />
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8, marginTop: 6 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <select value={classTeacherClassId} onChange={e => setClassTeacherClassId(e.target.value)}
                    style={{ padding: '6px 10px', borderRadius: 6, border: '1.5px solid #e5e7eb', fontSize: 12, outline: 'none', fontFamily: '"DM Sans",sans-serif', cursor: 'pointer' }}>
                    <option value="">None</option>
                    {(classes as any[]).filter(c => newClassIds.includes(c.id)).map(c => (
