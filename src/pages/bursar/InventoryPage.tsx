...
export default function InventoryPage() {
  ...
  const [tab, setTab] = useState<'inventory' | 'sales' | 'ledger'>('inventory')
      ...
      <div style={{ paddingBottom: 60, fontFamily: '"DM Sans", sans-serif', animation: '_an_fi .4s ease-out' }}>
        ...
        <div style={{ marginBottom: 28, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: 16 }}>
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            {tab === 'inventory' && (
              <button 
                onClick={() => setItemModal({ name: '', category: 'Uniform', cost_price: 0, selling_price: 0, current_stock: 0, reorder_level: 5, unit: 'pcs', school_id: schoolId })}
          ...
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(30,6,70,0.4)', backdropFilter: 'blur(6px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 20 }}>
             <div style={{ background: '#fff', borderRadius: 28, width: '100%', maxWidth: 440, padding: 32, animation: '_an_fi .3s ease-out' }}>
                ...
                <div style={{ display: 'flex', gap: 12, marginTop: 24 }}>
                   ...
                   <button 
                     onClick={() => recordSale.mutate({ school_id: schoolId, item_id: saleModal.item_id, student_id: saleModal.student_id, buyer_name: saleModal.buyer_name, quantity: saleModal.quantity, unit_price: saleModal.selling_price, total_amount: saleModal.total_amount, payment_method: saleModal.payment_method, recorded_by: user?.id })} 
