...
export default function InventoryPage() {
  ...
  const SUPPLY_CATEGORY_META: Record<string, { label: string; icon: any; color: string }> = {
    ...
    other: { label: 'Other Items', icon: Package, color: 'var(--text-muted)' },
  }
      ...
      <div style={{ paddingBottom: 60, fontFamily: '"DM Sans", sans-serif', animation: '_an_fi .4s ease-out' }}>
        ...
        <div style={{ marginBottom: 28, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: 16 }}>
          <div>
            ...
            <p style={{ color: 'var(--text-muted)', fontSize: 13, marginTop: 4 }}>Manage textbooks, uniforms, and stationery assets</p>
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: 16, marginBottom: 30 }}>
          ...
          <div style={{ background: 'var(--bg-card)', borderRadius: 20, padding: 22, border: '1.5px solid #f0eefe', display: 'flex', alignItems: 'center', gap: 16 }}>
             ...
             <div>
               ...
               <div style={{ fontSize: 12, color: 'var(--text-subtle)', fontWeight: 600 }}>Total Sales Recorded</div>
             </div>
          </div>
        </div>
        ...
        <div style={{ marginBottom: 24, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 16 }}>
          ...
          <div style={{ position: 'relative' }}>
            ...
            <input 
              ...
              style={{ width: 240, padding: '10px 16px 10px 40px', borderRadius: 12, border: '1.5px solid var(--border-color)', outline: 'none', fontSize: 13 }}
            />
          </div>
        </div>
        ...
        {loadingItems || loadingSales || loadingLogs ? (
          <div style={{ padding: '80px 0', textAlign: 'center' }}>
            <div style={{ width: 40, height: 40, border: '3px solid var(--border-light)', borderTopColor: '#6d28d9', borderRadius: '50%', animation: '_an_spin 1s linear infinite', margin: '0 auto 16px' }} />
            <p style={{ fontSize: 13, color: 'var(--text-muted)' }}>Fetching store data...</p>
          </div>
        ) : (
          <>
            {tab === 'inventory' && (
                  ...
                  <div key={item.id} style={{ background: 'var(--bg-card)', borderRadius: 24, border: '1.5px solid #f0eefe', padding: 24, transition: 'all .3s ease' }} className="inv-card">
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
                      ...
                      <div style={{ display: 'flex', gap: 6 }}>
                        <button onClick={() => setItemModal(item)} style={{ padding: 6, borderRadius: 8, border: 'none', background: 'var(--bg-input)', color: 'var(--text-muted)', cursor: 'pointer' }}><Edit size={14}/></button>
                    ...
                    <h3 style={{ fontSize: 17, fontWeight: 800, color: 'var(--text-main)', margin: '0 0 4px' }}>{item.name}</h3>
                    <p style={{ fontSize: 12, color: 'var(--text-subtle)', margin: '0 0 16px' }}>{item.description || 'No description provided'}</p>
                    
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 20 }}>
                      <div style={{ background: 'var(--bg-input)', padding: '10px 14px', borderRadius: 14 }}>
                        <div style={{ fontSize: 10, color: 'var(--text-subtle)', fontWeight: 600 }}>Stock Level</div>
                        <div style={{ fontSize: 16, fontWeight: 900, color: item.current_stock <= (item.reorder_level || 5) ? '#dc2626' : '#111827' }}>
                          {item.current_stock} <span style={{ fontSize: 11, color: 'var(--text-subtle)' }}>{item.unit}</span>
                        </div>
                      </div>
                      <div style={{ background: 'var(--bg-input)', padding: '10px 14px', borderRadius: 14 }}>
                        <div style={{ fontSize: 10, color: 'var(--text-subtle)', fontWeight: 600 }}>Retail Price</div>
                    ...
                    <div style={{ display: 'flex', gap: 10 }}>
                      ...
                      <button 
                        ...
                        style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '11px', background: 'var(--bg-card)', color: 'var(--text-main)', border: '1.5px solid var(--border-color)', borderRadius: 12, width: 44, cursor: 'pointer' }}
                        className="inv-btn"
                      >
                        <Truck size={15} />
                      </button>
              ...
              <div style={{ background: 'var(--bg-card)', borderRadius: 20, border: '1.5px solid #f0eefe', overflow: 'hidden' }}>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  ...
                  <tbody>
                      ...
                      return (
                        <tr key={item.id} style={{ borderBottom: '1px solid #f9fafb' }}>
                          ...
                          <td style={{ padding: '16px 20px' }}>
                            ...
                            <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>{item.quantity} {item.unit} {item.is_required ? '(Required)' : '(Optional)'}</div>
                          </td>
                          <td style={{ padding: '16px 20px' }}>
                          ...
                          <td style={{ padding: '16px 20px' }}>
                            {linkedItem ? (
                              ...
                              <span style={{ fontSize: 11, color: 'var(--text-subtle)' }}>No link</span>
                            )}
                          </td>
                    ...
                    {allSupplies.length === 0 && <tr><td colSpan={6} style={{ padding: 60, textAlign: 'center', color: 'var(--text-subtle)' }}>No welcome pack items found.</td></tr>}
                  </tbody>
                </table>
              </div>
            )}
                  ...
                  <tbody>
                    {sales.map(s => (
                      <tr key={s.id} style={{ borderBottom: '1px solid #f9fafb' }}>
                        <td style={{ padding: '14px 24px', fontSize: 13, color: 'var(--text-muted)' }}>{format(new Date(s.created_at), 'dd MMM, HH:mm')}</td>
                        <td style={{ padding: '14px 24px' }}>
                          <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--text-main)' }}>{s.item?.name}</div>
                          <div style={{ fontSize: 11, color: 'var(--text-subtle)' }}>{s.item?.category}</div>
                        </td>
                        <td style={{ padding: '14px 24px', fontSize: 13, color: 'var(--text-muted)' }}>{s.student?.full_name || s.buyer_name || 'Counter Sale'}</td>
                        ...
                        <td style={{ padding: '14px 24px' }}><button style={{ color: 'var(--text-subtle)', border: 'none', background: 'none' }}><ChevronRight size={16}/></button></td>
                      </tr>
                    ))}
                    {sales.length === 0 && <tr><td colSpan={6} style={{ padding: 60, textAlign: 'center', color: 'var(--text-subtle)' }}>No sales recorded yet</td></tr>}
                  </tbody>
                </table>
              </div>
            )}
                  ...
                  <tbody>
                    {logs.map(log => (
                      <tr key={log.id} style={{ borderBottom: '1px solid #f9fafb' }}>
                        <td style={{ padding: '14px 24px', fontSize: 13, color: 'var(--text-muted)' }}>{format(new Date(log.created_at), 'dd MMM, HH:mm')}</td>
                        <td style={{ padding: '14px 24px', fontSize: 14, fontWeight: 700, color: 'var(--text-main)' }}>{log.item?.name}</td>
                        <td style={{ padding: '14px 24px' }}>
                        ...
                        <td style={{ padding: '14px 24px', fontSize: 13, color: 'var(--text-subtle)' }}>{log.previous_stock}</td>
                        <td style={{ padding: '14px 24px', fontSize: 14, fontWeight: 800 }}>{log.new_stock}</td>
                        <td style={{ padding: '14px 24px', fontSize: 13, color: 'var(--text-muted)' }}>{log.reason}</td>
                      </tr>
                    ))}
                    {logs.length === 0 && <tr><td colSpan={6} style={{ padding: 60, textAlign: 'center', color: 'var(--text-subtle)' }}>No logs found</td></tr>}
                  </tbody>
                </table>
              </div>
            )}
          ...
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(30,6,70,0.4)', backdropFilter: 'blur(6px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 20 }}>
             <div style={{ background: 'var(--bg-card)', borderRadius: 28, width: '100%', maxWidth: 440, padding: 32, boxShadow: '0 25px 50px -12px rgba(0,0,0,0.15)', animation: '_an_fi .3s ease-out' }}>
                ...
                <div style={{ display: 'grid', gap: 16 }}>
                   <div>
                     <label style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-main)', marginBottom: 6, display: 'block' }}>Product Name</label>
                     <input value={itemModal.name} onChange={e => setItemModal({...itemModal, name: e.target.value})} style={{ width: '100%', padding: '12px', borderRadius: 12, border: '1.5px solid var(--border-color)', outline: 'none' }} />
                   </div>
                   <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                     <div>
                       <label style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-main)', marginBottom: 6, display: 'block' }}>Category</label>
                       <select value={itemModal.category} onChange={e => setItemModal({...itemModal, category: e.target.value})} style={{ width: '100%', padding: '12px', borderRadius: 12, border: '1.5px solid var(--border-color)', background: 'var(--bg-card)' }}>
                      ...
                      <div>
                         <label style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-main)', marginBottom: 6, display: 'block' }}>Unit</label>
                         <select value={itemModal.unit} onChange={e => setItemModal({...itemModal, unit: e.target.value})} style={{ width: '100%', padding: '12px', borderRadius: 12, border: '1.5px solid var(--border-color)', background: 'var(--bg-card)' }}>
                   ...
                   <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                     <div>
                        <label style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-main)', marginBottom: 6, display: 'block' }}>Cost Price</label>
                        <input type="number" value={itemModal.cost_price} onChange={e => setItemModal({...itemModal, cost_price: Number(e.target.value)})} style={{ width: '100%', padding: '12px', borderRadius: 12, border: '1.5px solid var(--border-color)' }} />
                     </div>
                     <div>
                        <label style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-main)', marginBottom: 6, display: 'block' }}>Selling Price</label>
                        <input type="number" value={itemModal.selling_price} onChange={e => setItemModal({...itemModal, selling_price: Number(e.target.value)})} style={{ width: '100%', padding: '12px', borderRadius: 12, border: '1.5px solid var(--border-color)' }} />
                     </div>
                   </div>
                   {!itemModal.id && (
                     <div>
                        <label style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-main)', marginBottom: 6, display: 'block' }}>Initial Stock</label>
                        <input type="number" value={itemModal.current_stock} onChange={e => setItemModal({...itemModal, current_stock: Number(e.target.value)})} style={{ width: '100%', padding: '12px', borderRadius: 12, border: '1.5px solid var(--border-color)' }} />
                     </div>
                   )}
                </div>
                <div style={{ display: 'flex', gap: 12, marginTop: 28 }}>
                   <button onClick={() => setItemModal(null)} style={{ flex: 1, padding: '12px', border: '1.5px solid var(--border-color)', background: 'var(--bg-card)', borderRadius: 12, fontWeight: 700, cursor: 'pointer' }}>Cancel</button>
        ...
        {saleModal && (
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(30,6,70,0.4)', backdropFilter: 'blur(6px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 20 }}>
             <div style={{ background: 'var(--bg-card)', borderRadius: 28, width: '100%', maxWidth: 440, padding: 32, animation: '_an_fi .3s ease-out' }}>
                ...
                <p style={{ fontSize: 14, color: 'var(--text-main)', fontWeight: 800, marginBottom: 20, background: '#f5f3ff', padding: '10px 16px', borderRadius: 12 }}>Item: {saleModal.name}</p>
                
                <div style={{ display: 'grid', gap: 16 }}>
                   <div>
                     <label style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-main)', marginBottom: 6, display: 'block' }}>Sale Type / Customer</label>
                     <select 
                   ...
                   <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                     <div>
                       <label style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-main)', marginBottom: 6, display: 'block' }}>Quantity</label>
                       <input 
                         ...
                         style={{ width: '100%', padding: '12px', borderRadius: 12, border: '1.5px solid var(--border-color)' }} 
                       />
                       <span style={{ fontSize: 11, color: 'var(--text-subtle)' }}>Max available: {saleModal.current_stock}</span>
                     </div>
                     <div>
                       <label style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-main)', marginBottom: 6, display: 'block' }}>Payment Method</label>
                       <select value={saleModal.payment_method} onChange={e => setSaleModal({...saleModal, payment_method: e.target.value})} style={{ width: '100%', padding: '12px', borderRadius: 12, border: '1.5px solid var(--border-color)', background: 'var(--bg-card)' }}>
                ...
                <div style={{ display: 'flex', gap: 12, marginTop: 24 }}>
                   <button onClick={() => setSaleModal(null)} style={{ flex: 1, padding: '12px', border: '1.5px solid var(--border-color)', background: 'var(--bg-card)', borderRadius: 12, fontWeight: 700, cursor: 'pointer' }}>Cancel</button>
                   <button 
          ...
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(30,6,70,0.4)', backdropFilter: 'blur(6px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 20 }}>
             <div style={{ background: 'var(--bg-card)', borderRadius: 28, width: '100%', maxWidth: 500, padding: 32, animation: '_an_fi .3s ease-out' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                    <div>
                       <label style={{ fontSize: 11, fontWeight: 800, color: 'var(--text-subtle)', textTransform: 'uppercase', marginBottom: 6, display: 'block' }}>Category</label>
                       <select value={supplyModal.category} onChange={e => setSupplyModal({...supplyModal, category: e.target.value})} style={{ width: '100%', padding: '12px', borderRadius: 12, border: '1.5px solid var(--border-color)', background: 'var(--bg-card)' }}>
                  ...
                  <div>
                     <label style={{ fontSize: 11, fontWeight: 800, color: 'var(--text-subtle)', textTransform: 'uppercase', marginBottom: 6, display: 'block' }}>Item Name / Description</label>
                     <input value={supplyModal.item_name} onChange={e => setSupplyModal({...supplyModal, item_name: e.target.value})} placeholder="e.g. Science Textbook 1" style={{ width: '100%', padding: '12px', borderRadius: 12, border: '1.5px solid var(--border-color)', boxSizing: 'border-box' }} />
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                    <div>
                       <label style={{ fontSize: 11, fontWeight: 800, color: 'var(--text-subtle)', textTransform: 'uppercase', marginBottom: 6, display: 'block' }}>Qty</label>
                       <input type="number" value={supplyModal.quantity} onChange={e => setSupplyModal({...supplyModal, quantity: Number(e.target.value)})} style={{ width: '100%', padding: '12px', borderRadius: 12, border: '1.5px solid var(--border-color)', boxSizing: 'border-box' }} />
                    </div>
                    <div>
                       <label style={{ fontSize: 11, fontWeight: 800, color: 'var(--text-subtle)', textTransform: 'uppercase', marginBottom: 6, display: 'block' }}>Unit</label>
                       <input value={supplyModal.unit} onChange={e => setSupplyModal({...supplyModal, unit: e.target.value})} placeholder="copy, pcs, ream" style={{ width: '100%', padding: '12px', borderRadius: 12, border: '1.5px solid var(--border-color)', boxSizing: 'border-box' }} />
                    </div>
                  </div>
                  <div>
                     <label style={{ fontSize: 11, fontWeight: 800, color: 'var(--text-subtle)', textTransform: 'uppercase', marginBottom: 6, display: 'block' }}>Connect to School Store</label>
                     <select value={supplyModal.inventory_item_id || ''} onChange={e => setSupplyModal({...supplyModal, inventory_item_id: e.target.value || null})} style={{ width: '100%', padding: '12px', borderRadius: 12, border: '1.5px solid var(--border-color)', background: 'var(--bg-card)' }}>
                  ...
                  <div style={{ display: 'flex', gap: 12, marginTop: 10 }}>
                    <button onClick={() => setSupplyModal(null)} style={{ flex: 1, padding: '12px', border: '1.5px solid var(--border-color)', background: 'var(--bg-card)', borderRadius: 12, fontWeight: 700, cursor: 'pointer' }}>Cancel</button>
                    <button 
        ...
        {restockModal && (
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(30,6,70,0.4)', backdropFilter: 'blur(6px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 20 }}>
             <div style={{ background: 'var(--bg-card)', borderRadius: 28, width: '100%', maxWidth: 440, padding: 32, animation: '_an_fi .3s ease-out' }}>
                ...
                <div style={{ display: 'grid', gap: 16 }}>
                   <div>
                     <label style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-main)', marginBottom: 6, display: 'block' }}>Add Quantity</label>
                     <input type="number" value={restockModal.add} onChange={e => setRestockModal({...restockModal, add: e.target.value})} style={{ width: '100%', padding: '12px', borderRadius: 12, border: '1.5px solid var(--border-color)' }} />
                   </div>
                   <div>
                      <label style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-main)', marginBottom: 6, display: 'block' }}>Reason / Note</label>
                      <input value={restockModal.reason} onChange={e => setRestockModal({...restockModal, reason: e.target.value})} style={{ width: '100%', padding: '12px', borderRadius: 12, border: '1.5px solid var(--border-color)' }} />
                   </div>
                </div>
                <div style={{ display: 'flex', gap: 12, marginTop: 28 }}>
                   <button onClick={() => setRestockModal(null)} style={{ flex: 1, padding: '12px', border: '1.5px solid var(--border-color)', background: 'var(--bg-card)', borderRadius: 12, fontWeight: 700, cursor: 'pointer' }}>Cancel</button>
                   <button onClick={() => {
