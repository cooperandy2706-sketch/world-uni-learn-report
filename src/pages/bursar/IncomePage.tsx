    ...
    <>
      ...
      <div style={{ fontFamily: '"DM Sans",system-ui,sans-serif', animation: '_inc_fi .4s ease' }}>
        <div style={{ marginBottom: 24, display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
          <div>
            <h1 style={{ fontFamily: '"Playfair Display",serif', fontSize: 26, fontWeight: 700, color: 'var(--text-main)', margin: 0 }}>Income Records</h1>
            <p style={{ fontSize: 13, color: 'var(--text-muted)', marginTop: 3 }}>All income sources — {CUR(totalIncome)}</p>
          </div>
        ...
        {/* Add form */}
        {showForm && (
          <div style={{ background: 'var(--bg-card)', borderRadius: 18, padding: 24, border: '1.5px solid #bbf7d0', marginBottom: 24, boxShadow: '0 2px 12px rgba(22,163,74,.08)' }}>
            <h3 style={{ fontFamily: '"Playfair Display",serif', fontSize: 16, fontWeight: 700, color: 'var(--text-main)', margin: '0 0 16px' }}>Record New Income</h3>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
              {[
                ...
                { label: 'Reference No.', content: <input value={form.reference} onChange={e => setForm(p => ({ ...p, reference: e.target.value }))} placeholder="Optional" style={{ width: '100%', padding: '9px 12px', borderRadius: 9, border: '1.5px solid var(--border-color)', fontSize: 13, outline: 'none', boxSizing: 'border-box' }} /> },
                { label: 'Description', content: <input value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))} placeholder="Optional note" style={{ width: '100%', padding: '9px 12px', borderRadius: 9, border: '1.5px solid var(--border-color)', fontSize: 13, outline: 'none', boxSizing: 'border-box' }} />, span: true },
              ].map(f => (
          ...
          {isLoading ? <div style={{ padding: '40px', textAlign: 'center', color: 'var(--text-subtle)' }}>Loading…</div>
            : filtered.length === 0 ? (
              <div style={{ padding: '60px', textAlign: 'center' }}>
                ...
                <p style={{ fontSize: 13, color: 'var(--text-subtle)' }}>No income records for {year}{filterCat ? ` · ${filterCat}` : ''}</p>
              </div>
            ) : (
                ...
                <tbody>
                  {filtered.map((r: any) => (
                    <tr key={r.id} className="inc-row" style={{ borderBottom: '1px solid #f0fdf4', transition: 'background .12s' }}>
                      ...
                      <td style={{ padding: '11px 16px', fontSize: 13, color: 'var(--text-main)' }}>{r.description ?? '—'}</td>
                      <td style={{ padding: '11px 16px', fontSize: 12, color: 'var(--text-subtle)', fontFamily: 'monospace' }}>{r.reference ?? '—'}</td>
