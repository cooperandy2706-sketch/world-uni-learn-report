...
export default function IncidentsPage() {
  ...
  // Store incidents in gate_scans-adjacent table — use a simple local table approach via Supabase
  // We'll use a school_assets-like generic approach with the notifications table for now
  ...
  return (
    <>
      ...
      <div style={{ fontFamily: '"DM Sans",sans-serif', paddingBottom: 100 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
          <div>
            <h1 style={{ fontSize: 26, fontWeight: 900, color: '#0f172a', margin: 0 }}>Incident Reports</h1>
            <p style={{ fontSize: 13, color: '#64748b', margin: '4px 0 0' }}>Log and track security incidents</p>
          </div>
          <button onClick={() => setOpen(true)} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '11px 20px', borderRadius: 12, border: 'none', background: '#dc2626', color: '#fff', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>
            <Plus size={15} /> Report
          </button>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          : incidents.length === 0 ? (
            <div style={{ padding: '48px', textAlign: 'center', color: '#94a3b8', background: '#fff', borderRadius: 20, border: '1.5px solid #f1f5f9' }}>
              <AlertTriangle size={40} style={{ marginBottom: 10, opacity: .3 }} />
              <div style={{ fontSize: 15, fontWeight: 700, color: '#334155' }}>No incidents recorded</div>
            ...
            return (
              <div key={inc.id} style={{ background: '#fff', borderRadius: 18, border: '1.5px solid #f1f5f9', padding: '18px 20px' }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 10, marginBottom: 10 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    ...
                    <div>
                      <div style={{ fontSize: 15, fontWeight: 800, color: '#0f172a' }}>{meta.type ?? 'Incident'}</div>
                      <div style={{ fontSize: 11, color: '#94a3b8' }}>{format(new Date(inc.created_at), 'MMM d, yyyy · hh:mm a')}</div>
                    </div>
                  </div>
                  <span style={{ fontSize: 11, fontWeight: 700, padding: '4px 10px', borderRadius: 99, background: sev.bg, color: sev.color, whiteSpace: 'nowrap' }}>
                    {sev.label} Severity
                  </span>
                </div>
        ...
        {/* Modal */}
        {open && (
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'flex-end', justifyContent: 'center', zIndex: 999, padding: '0 0 0' }}>
            <div style={{ background: '#fff', borderRadius: '24px 24px 0 0', padding: 28, width: '100%', maxWidth: 520, maxHeight: '90vh', overflowY: 'auto' }}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  {SEVERITY.map(s => (
                    <button key={s.value} onClick={() => setForm(f => ({ ...f, severity: s.value }))}
                      style={{ flex: 1, padding: '10px', borderRadius: 10, border: `2px solid ${form.severity === s.value ? s.color : '#e2e8f0'}`, background: form.severity === s.value ? s.bg : '#fff', color: s.color, fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>
                      {s.label}
                    </button>
                  ))}
                </div>
              ...
              <button onClick={submit} disabled={saving} style={{ width: '100%', padding: '14px', borderRadius: 14, border: 'none', background: '#dc2626', color: '#fff', fontSize: 14, fontWeight: 800, cursor: 'pointer', marginTop: 4 }}>
