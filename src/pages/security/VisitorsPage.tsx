...
export default function VisitorsPage() {
  ...
  async function signOut(id: string, name: string) {
    await supabase.from('visitors').update({ time_out: new Date().toISOString() }).eq('id', id)
    ...
    <>
      ...
      <div style={{ fontFamily: '"DM Sans",sans-serif', paddingBottom: 100 }}>
        <div style={{ marginBottom: 24 }}>
          <h1 style={{ fontSize: 26, fontWeight: 900, color: '#0f172a', margin: 0 }}>Visitor Log</h1>
        ...
        <div style={{ display: 'flex', gap: 10, marginBottom: 16 }}>
          <div style={{ flex: 1, position: 'relative' }}>
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search visitors…"
              style={{ width: '100%', padding: '11px 12px 11px 34px', borderRadius: 12, border: '1.5px solid #e2e8f0', fontSize: 13, outline: 'none', boxSizing: 'border-box' }} />
          </div>
          <button onClick={() => setAddOpen(true)} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '0 20px', borderRadius: 12, border: 'none', background: '#0f172a', color: '#fff', fontSize: 13, fontWeight: 700, cursor: 'pointer', whiteSpace: 'nowrap' }}>
        ...
        <div style={{ background: '#fff', borderRadius: 20, border: '1.5px solid #f1f5f9', overflow: 'hidden' }}>
          {isLoading ? (
            ...
            <div style={{ padding: '48px 24px', textAlign: 'center', color: '#94a3b8' }}>
              <Users size={40} style={{ marginBottom: 10, opacity: .3 }} />
              <div style={{ fontSize: 15, fontWeight: 700, color: '#334155', marginBottom: 4 }}>No visitors today</div>
              <div style={{ fontSize: 13 }}>Click "Sign In Visitor" to register a new arrival</div>
            </div>
          ) : filtered.map(v => (
            <div key={v.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 18px', borderBottom: '1px solid #f8fafc' }}>
              <div style={{ width: 44, height: 44, borderRadius: '50%', flexShrink: 0, background: v.time_out ? '#f1f5f9' : '#dcfce7', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, fontWeight: 900, color: v.time_out ? '#94a3b8' : '#059669' }}>
              ...
              <div style={{ flexShrink: 0 }}>
                {!v.time_out ? (
                  <button onClick={() => signOut(v.id, v.full_name)}
                    style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '7px 14px', borderRadius: 9, border: 'none', background: '#fef2f2', color: '#dc2626', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>
        ...
        {addOpen && (
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'flex-end', zIndex: 999 }}>
            <div style={{ background: '#fff', borderRadius: '24px 24px 0 0', padding: 28, width: '100%', maxHeight: '90vh', overflowY: 'auto', fontFamily: '"DM Sans",sans-serif' }}>
              ...
              <Field label="Purpose of Visit" value={form.purpose} onChange={(v: string) => setForm(f => ({ ...f, purpose: v }))} placeholder="Parent meeting, Delivery, Interview…" />
