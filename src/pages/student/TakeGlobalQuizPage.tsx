  ...
  if (status === 'prep') {
    return (
      <div style={{ maxWidth: 640, margin: '60px auto', background: 'var(--bg-card)', borderRadius: 12, padding: 40, border: '1.5px solid #f0eefe', textAlign: 'center', boxShadow: '0 20px 50px rgba(109,40,217,0.1)' }}>
        ...
        <h1 style={{ fontFamily: '"Playfair Display", serif', fontSize: 32, fontWeight: 700, color: 'var(--text-main)', marginBottom: 12 }}>{quiz.title}</h1>
  ...
  if (status === 'finished') {
    ...
    return (
      <div style={{ maxWidth: 700, margin: '40px auto 100px', fontFamily: '"DM Sans",sans-serif' }}>
        ...
        <h3 style={{ fontSize: 18, fontWeight: 700, color: 'var(--text-main)', marginBottom: 20, paddingLeft: 8 }}>📝 Your Answer Breakdown</h3>
          ...
          {questions.map((q, i) => {
            ...
            return (
              <div key={q.id} style={{ background: 'var(--bg-card)', border: `1.5px solid ${isCorrect ? '#a7f3d0' : '#fecaca'}`, borderRadius: 8, padding: '20px 24px', position: 'relative', overflow: 'hidden' }}>
                ...
                <div className="resp-grid-2" style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) minmax(0, 1fr)', gap: 12, background: 'var(--bg-input)', padding: 14, borderRadius: 12 }}>
                  <div>
  ...
  return (
    <>
      ...
      <div style={{ position: 'sticky', top: 0, zIndex: 100, background: 'rgba(255,255,255,0.98)', backdropFilter: 'blur(12px)', borderBottom: '1.5px solid #f0eefe', padding: '10px 0' }}>
        <div style={{ maxWidth: 800, margin: '0 auto', padding: '0 16px', display: 'flex', gap: 12, justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ minWidth: 0 }}>
            ...
            <h2 style={{ fontSize: 14, fontWeight: 700, color: 'var(--text-main)', margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{quiz.title}</h2>
          </div>
      ...
      <div style={{ maxWidth: 650, margin: '50px auto 80px', padding: '0 20px', animation: '_fade 0.3s ease' }}>
        ...
        <h1 style={{ fontFamily: '"Playfair Display", serif', fontSize: 26, fontWeight: 700, color: 'var(--text-main)', marginBottom: 36, lineHeight: 1.4 }}>{q.text}</h1>
