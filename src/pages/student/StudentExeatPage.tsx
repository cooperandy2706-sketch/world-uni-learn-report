...
export default function StudentExeatPage() {
  const { user } = useAuth()
  const studentId = user?.metadata?.link_id
  ...
  const { data: exeats = [] } = useQuery({
    ...
    queryFn: async () => {
      if (!studentId) return []
      const { data } = await boardingService.getStudentExeats(studentId)
      return data || []
    },
    enabled: !!studentId
  })
  ...
  const createExeat = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['student-exeats'] }); setModal(false); toast.success('Exeat request submitted') }
  })

  const [modal, setModal] = useState(false)
  const [form, setForm] = useState({ reason: '', destination: '', departure_time: '', expected_return_time: '' })

  if (!studentId) {
    return <div style={{ padding: 32, textAlign: 'center', color: '#64748b' }}>Student profile not properly linked. Contact admin.</div>
  }
  ...
  return (
    <div style={{ padding: 32, maxWidth: 1200, margin: '0 auto', fontFamily: '"DM Sans",sans-serif' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 28, fontWeight: 800, color: '#1e0646', margin: '0 0 8px' }}>My Exeats</h1>
          <p style={{ color: '#64748b', margin: 0, fontSize: 14 }}>Request permission to leave campus.</p>
        </div>
      ...
      <Modal open={modal} onClose={() => setModal(false)} title="Request Exeat" size="sm" footer={<><Btn variant="secondary" onClick={() => setModal(false)}>Cancel</Btn><Btn onClick={() => { if(!form.destination || !form.departure_time || !form.expected_return_time) return toast.error('Fill required fields'); createExeat.mutate({ school_id: schoolId, student_id: studentId, requested_by: user?.id, reason: form.reason, destination: form.destination, departure_time: new Date(form.departure_time).toISOString(), expected_return_time: new Date(form.expected_return_time).toISOString(), parent_notified: false, security_notified: false, status: 'pending' }) }}><Send size={16} /> Submit</Btn></>}>
