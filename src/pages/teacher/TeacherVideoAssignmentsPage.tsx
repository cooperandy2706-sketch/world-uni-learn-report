    ...
    <>
      ...
      <div style={{ padding: '32px 40px', maxWidth: 1200, margin: '0 auto', fontFamily: '"DM Sans",sans-serif' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 32 }}>
          <div>
            <h1 style={{ fontSize: 28, fontWeight: 800, color: '#111827', margin: '0 0 8px' }}>Video Assignments</h1>
            <p style={{ fontSize: 15, color: '#6b7280', margin: 0 }}>Assign YouTube videos to your classes and track who watched them.</p>
          </div>
        ...
        {loading ? (
          <div style={{ textAlign: 'center', padding: '60px', color: '#9ca3af' }}>Loading assignments...</div>
        ) : assignments.length === 0 ? (
          <div style={{ background: '#fff', borderRadius: 24, padding: '60px 20px', textAlign: 'center', border: '1.5px solid #f0eefe' }}>
            <div style={{ fontSize: 48, marginBottom: 16 }}>📹</div>
            <h3 style={{ fontSize: 18, fontWeight: 700, color: '#111827', margin: '0 0 8px' }}>No video assignments yet</h3>
            <p style={{ fontSize: 14, color: '#6b7280', margin: '0 0 24px' }}>Share educational YouTube videos with your students.</p>
            <button onClick={() => setShowModal(true)} style={{ padding: '10px 20px', background: '#f8fafc', color: '#111827', border: '1.5px solid #e2e8f0', borderRadius: 10, fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>
              Assign a Video
            </button>
          </div>
        ) : (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: 20 }}>
            {assignments.map(a => {
              ...
              return (
                <div key={a.id} style={{ background: '#fff', borderRadius: 20, overflow: 'hidden', border: '1.5px solid #f0eefe', display: 'flex', flexDirection: 'column' }}>
                  ...
                  <div style={{ padding: '20px', flex: 1, display: 'flex', flexDirection: 'column' }}>
                    <h3 style={{ fontSize: 16, fontWeight: 700, color: '#111827', margin: '0 0 8px', lineHeight: 1.4 }}>{a.title}</h3>
                    {a.instructions && (
                      <p style={{ fontSize: 13, color: '#6b7280', margin: '0 0 16px', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' as any }}>
                        {a.instructions}
                      </p>
                    )}
      ...
      {showModal && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 1000, background: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div style={{ background: '#fff', width: '100%', maxWidth: 500, borderRadius: 24, overflow: 'hidden', animation: '_slideUp 0.3s ease' }}>
            ...
            <form onSubmit={handleSubmit} style={{ padding: 24 }}>
              ...
              <div style={{ display: 'flex', gap: 12, marginBottom: 16 }}>
                <div style={{ flex: 1 }}>
                  ...
                  <select required value={classId} onChange={e => setClassId(e.target.value)} style={{ width: '100%', padding: '12px', borderRadius: 10, border: '1.5px solid #e2e8f0', fontSize: 14, outline: 'none', background: '#fff' }}>
