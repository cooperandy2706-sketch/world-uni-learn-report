  ...
  if (status === 'prep') {
    return (
      ...
      <div style={{ maxWidth: 640, margin: '0 auto', background: 'var(--bg-card)', borderRadius: 12, padding: 40, border: '1.5px solid #f0eefe', textAlign: 'center', boxShadow: '0 20px 50px rgba(109,40,217,0.1)' }}>
        ...
        <p style={{ fontSize: 15, color: 'var(--text-muted)', marginBottom: 32, lineHeight: 1.6 }}>{quiz.description || 'Preview this global interactive quiz.'} <br/> <strong style={{color: '#7c3aed'}}>Note:</strong> Preview submissions are not recorded to the database.</p>
        ...
        <div className="resp-2col-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 40 }}>
          <div style={{ background: '#f5f3ff', borderRadius: 8, padding: 16 }}>
            <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-subtle)', textTransform: 'uppercase', marginBottom: 4 }}>Questions</div>
            <div style={{ fontSize: 24, fontWeight: 800, color: '#7c3aed' }}>{questions.length} Items</div>
          </div>
          <div style={{ background: '#f5f3ff', borderRadius: 8, padding: 16 }}>
            <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-subtle)', textTransform: 'uppercase', marginBottom: 4 }}>Time Limit</div>
  ...
  if (status === 'finished') {
    ...
    return (
      ...
      <div style={{ maxWidth: 700, margin: '0 auto', fontFamily: '"DM Sans",sans-serif' }}>
        ...
        <h3 style={{ fontSize: 18, fontWeight: 700, color: 'var(--text-main)', marginBottom: 20, paddingLeft: 8 }}>📝 Answer Breakdown</h3>
            ...
            return (
              <div key={q.id} style={{ background: 'var(--bg-card)', border: `1.5px solid ${isCorrect ? '#a7f3d0' : '#fecaca'}`, borderRadius: 8, padding: '20px 24px', position: 'relative', overflow: 'hidden' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
                  <div style={{ fontSize: 12, fontWeight: 800, color: 'var(--text-subtle)', textTransform: 'uppercase' }}>Question {i + 1}</div>
                ...
                <h4 style={{ fontSize: 16, fontWeight: 600, color: 'var(--text-main)', margin: '0 0 16px 0', lineHeight: 1.4 }}>{q.text}</h4>

                <div className="resp-2col-grid" style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) minmax(0, 1fr)', gap: 12, background: 'var(--bg-input)', padding: 14, borderRadius: 12 }}>
                  <div>
                    <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-subtle)', textTransform: 'uppercase', marginBottom: 6 }}>Your Answer</div>
                    <div style={{ fontSize: 14, fontWeight: 600, color: isCorrect ? '#059669' : '#dc2626' }}>
                      {userAnswers[q.id] || <span style={{ color: 'var(--text-subtle)', fontStyle: 'italic' }}>No Answer</span>}
                    </div>
                  </div>
                  {!isCorrect && (
                    <div style={{ borderLeft: '1px solid var(--border-color)', paddingLeft: 12 }}>
                      <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-subtle)', textTransform: 'uppercase', marginBottom: 6 }}>Correct Answer</div>
    ...
    <div className="t-page">
      ...
      <div style={{ position: 'sticky', top: 0, zIndex: 10, background: 'rgba(255,255,255,0.95)', backdropFilter: 'blur(10px)', borderBottom: '1.5px solid #f0eefe', padding: '14px 0' }}>
        <div className="t-header" style={{ maxWidth: 800, margin: '0 auto', padding: '0 20px' }}>
          <div>
            ...
            <h2 style={{ fontSize: 16, fontWeight: 700, color: 'var(--text-main)', margin: 0 }}>{quiz.title}</h2>
          </div>
      ...
      <div style={{ maxWidth: 650, margin: '50px auto 80px', padding: '0 20px', animation: '_fade 0.3s ease' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
          ...
          <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-subtle)' }}>{q.points} Points</div>
        </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '2px dashed var(--border-light)', paddingTop: 24 }}>
          <button 
            disabled={currentIdx === 0} 
            onClick={() => setCurrentIdx(prev => prev - 1)}
            style={{ padding: '12px 24px', border: '1.5px solid var(--border-color)', background: currentIdx === 0 ? '#f9fafb' : '#fff', borderRadius: 12, fontSize: 14, fontWeight: 700, color: currentIdx === 0 ? '#d1d5db' : '#4b5563', cursor: currentIdx === 0 ? 'not-allowed' : 'pointer', transition: 'all 0.2s' }}
          >← Previous</button>
