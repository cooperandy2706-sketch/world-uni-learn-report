                    ...
                    <div style={{ background: '#fff', borderRadius: 20, border: '1.5px solid #e5e7eb', padding: 20, animation: '_syl_fu 0.3s ease', boxShadow: '0 4px 6px rgba(0,0,0,0.02)' }}>
                      <div style={{ paddingBottom: 16, borderBottom: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16, flexWrap: 'wrap', gap: 12 }}>
                        <div>
                          ...
                          <h2 style={{ margin: 0, fontSize: 18, fontWeight: 800, color: '#1e293b' }}>{viewingFile.name}</h2>
                          <p style={{ margin: 0, fontSize: 12, color: '#64748b' }}>Curriculum Scheme Preview</p>
                        </div>
                        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                          <a href={viewingFile.url} download target="_blank" rel="noreferrer" style={{ display: 'inline-flex', alignItems: 'center', gap: 6, border: '1.5px solid #ddd6fe', background: '#f5f3ff', color: '#6d28d9', padding: '8px 14px', borderRadius: 10, fontSize: 12, fontWeight: 700, textDecoration: 'none', transition: 'all 0.2s' }}>
                            ⬇️ Download File
                          </a>
                          <button onClick={() => setViewingFile(null)} style={{ background: '#f1f5f9', border: 'none', width: 32, height: 32, borderRadius: '50%', fontSize: 20, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>×</button>
                        </div>
                      </div>
                      <div style={{ background: '#f8fafc', borderRadius: 14, overflow: 'hidden', height: '600px', border: '1.5px solid #f1f5f9' }}>
                        <iframe src={viewingFile.url} style={{ width: '100%', height: '100%', border: 'none' }} title={viewingFile.name} />
                      </div>
                    </div>
                ) : (
                    /* ── REGULAR FILES GRID VIEW ── */
                    <>
