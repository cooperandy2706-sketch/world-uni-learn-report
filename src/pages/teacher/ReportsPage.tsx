...
function ordinalFn(n:number){ const s=['th','st','nd','rd'],v=n%100; return n+(s[(v-20)%10]||s[v]||s[0]) }

function printReport(studentName:string){
  const el = document.getElementById('tr-report-print')
  if(!el) return
...
function Btn({ children, onClick, variant='primary', disabled, loading, style }:any){
  ...
  return (
    <button onClick={onClick} disabled={disabled}
      ...
      style={{ display:'inline-flex', alignItems:'center', gap:7, padding:'9px 16px', borderRadius:9, fontSize:13, fontWeight:600, cursor:disabled?'not-allowed':'pointer', transition:'all .15s', opacity:disabled?0.55:1, fontFamily:'"DM Sans",sans-serif', ...v[variant], ...style }}>
...
export default function TeacherReportsPage() {
  ...
  async function saveRemarks() {
    ...
    try {
      ...
      setRemarksDirty(false)
      toast.success('Remarks saved to report card ✓')
      // refresh
      await loadReport()
    } catch(e:any) {
  ...
  async function saveAndNotifyAdmin() {
    ...
    try {
      ...
      await supabase.from('messages').insert({
        ...
        subject: `Report ready: ${selectedStudent?.full_name} — ${(classOptions as any[]).find(c=>c.id===selectedClass)?.name}`,
        body: `Remarks have been completed for ${selectedStudent?.full_name}'s report card.\n\nClass Teacher: ${teacherRemark || 'Not set'}\nHeadteacher: ${htRemark || 'Not set'}\n\nPlease review and approve from the Reports section.`,
        priority: 'normal',
      })
  ...
  // build a fake report object for the ReportCard component
    ...
    <>
      ...
      <div style={{ fontFamily:'"DM Sans",system-ui,sans-serif', animation:'_rp_fi .4s ease' }}>
        ...
        <div style={{ marginBottom:22, display:'flex', alignItems:'flex-start', justifyContent:'space-between', flexWrap:'wrap', gap:12 }}>
          ...
          {/* Action buttons — only when student + report loaded */}
          {fakeReport && (
            <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
              ...
              <Btn variant="info" onClick={() => printReport(selectedStudent?.full_name ?? '')}>
                🖨️ Print A4
              </Btn>
        ...
        <div style={{ background:'#fef2f2', border:'1.5px solid #fca5a5', borderRadius:12, padding:'10px 16px', marginBottom:18, display:'flex', alignItems:'center', gap:10, fontSize:12 }}>
          ...
          <p style={{ margin:0, color:'#dc2626', fontWeight:500 }}>
            The admin portal is restricted to administrators only. Use <strong>Save & Notify Admin</strong> to send your completed remarks to the admin for approval and printing.
          </p>
        </div>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
              ...
              <div style={{ background:'#fff', borderRadius:16, border:'1.5px solid #f0eefe', overflow:'hidden', boxShadow:'0 1px 4px rgba(109,40,217,.06)' }}>
                ...
                <div style={{ padding:'0 18px 16px', display:'flex', gap:8, flexWrap:'wrap' }}>
                  <Btn variant="success" onClick={saveRemarks} loading={savingRemarks} disabled={!reportCard?.id}
                    style={{ flex:1, justifyContent:'center' }}>
                    💾 Save Remarks
                  </Btn>
                  <Btn variant="info" onClick={()=>setPreviewOpen(true)} disabled={!reportCard}
                    style={{ flex:1, justifyContent:'center' }}>
                    👁️ Preview Report
                  </Btn>
                  <Btn variant="orange" onClick={saveAndNotifyAdmin} loading={savingRemarks} disabled={!reportCard?.id}
                ...
                {!reportCard && (
                  <div style={{ margin:'0 18px 16px', background:'#fef2f2', border:'1px solid #fecaca', borderRadius:9, padding:'8px 12px', fontSize:12, color:'#dc2626' }}>
                    ⚠ No report card found for this student. Ask the admin to generate reports from the Reports section first.
                  </div>
                )}
              </div>
            </div>
      ...
      {/* ── Hidden print area uses real ReportCard ── */}
      <div id="tr-report-print" style={{ display:'none' }}>
        {fakeReport && (
          <ReportCard
            report={fakeReport}
            school={school}
      ...
      {/* ── Preview modal uses real ReportCard ── */}
      <Modal
        open={previewOpen}
        onClose={()=>setPreviewOpen(false)}
        title={`Report Card Preview — ${selectedStudent?.full_name ?? ''}`}
