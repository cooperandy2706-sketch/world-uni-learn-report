      ...
      <div style={{fontFamily:'"DM Sans",system-ui,sans-serif',animation:'_nt_fi .4s ease'}}>
        ...
        <div style={{marginBottom:22,display:'flex',alignItems:'flex-start',justifyContent:'space-between',flexWrap:'wrap',gap:12}}>
          <div>
            ...
            <p style={{fontSize:13,color: 'var(--text-muted)',marginTop:3}}>Your notifications and announcements from admin</p>
          </div>
          {unreadCount>0&&(
            <button onClick={markAllRead}
              style={{padding:'8px 14px',borderRadius:9,border: '1.5px solid var(--border-color)',background: 'var(--bg-card)',fontSize:12,fontWeight:600,cursor:'pointer',fontFamily:'"DM Sans",sans-serif',color: 'var(--text-main)'}}>
              ✓ Mark all read
            </button>
          )}
        </div>
                ...
                <div key={n.id} className="nt-item" onClick={()=>markRead(n.id)}
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:2}}>
                      <h4 style={{fontSize:13,fontWeight:700,color: 'var(--text-main)',margin:0}}>{n.title}</h4>
                      {!n.is_read&&<span style={{width:7,height:7,borderRadius:'50%',background:'#6d28d9',flexShrink:0,display:'block'}}/>}
                    </div>
                    {n.body&&<p style={{fontSize:12,color: 'var(--text-main)',margin:'0 0 4px',lineHeight:1.5}}>{n.body}</p>}
                    <span style={{fontSize:11,color: 'var(--text-subtle)'}}>{new Date(n.created_at).toLocaleString()}</span>
                  </div>
                </div>
              ))}
            </div>
                  ...
                  <div key={a.id} onClick={()=>markAnnouncementRead(a.id)}
                    ...
                    <div style={{display:'flex',alignItems:'flex-start',gap:12}}>
                      ...
                      <div style={{flex:1}}>
                        ...
                        <p style={{fontSize:13,color: 'var(--text-main)',margin:'0 0 6px',lineHeight:1.5}}>{a.body}</p>
                        {a.meeting_date&&(
